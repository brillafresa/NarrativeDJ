...
| Audio control parser | `DjAudioControlParserTest` (ko/en fallback, SSML strip via `spokenText`) |
...
| Production pipeline | `DjPipeline.runStorySegment` → OpenAI TTS or Android TTS → `audio-ducking.js` |

Production LLM/TTS uses on-device HTTP (Gemini/OpenAI) with keys in `SecureKeyStore` / `B2bLicenseStore`. No API keys in repo.

**Flow:** story → BYOK LLM (profile + current/target track context) → duck in → TTS → duck out → refresh cushion hint.
