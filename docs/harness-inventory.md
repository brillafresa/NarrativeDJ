...
## Cushion playback harness (Phase B)

| Asset | Verify command |
|-------|----------------|
| Playback order fixture | `harness/tests/mock_cushion_playback.json` — bridge IDs + `search_query` per track |
| JVM controller | `./gradlew test` → `CushionPlaybackControllerTest` |
| Instrumentation | `run_instrumentation.py` → `YtmControllerFixtureTest` (search/play on DOM fixture) |
| Production path | `CushionPlaybackController` → `NarrativeDJYtm.searchAndPlay(query)` in WebView |

**Flow:** Plan cushion route → Execute plays bridges sequentially via YTM search (catalog `search_query` fallback: title).
...
| Transition prompt | `DjTransitionPromptBuilder.build(...)` — Korean/English |
| Transition ment fallback | `DjAudioControlParserTest.fallbackForTransition` |
| Encrypted keys | `SecureKeyStoreTest` (instrumentation) |
| Metadata | `PlaybackMetadataFormatterTest` — notification / MediaSession labels |
| **Production pipeline** | `DjPipeline.runTransitionMent` → BYOK LLM or local fallback → OpenAI TTS or Android TTS → `audio-ducking.js` |

Production LLM/TTS uses on-device HTTP (Gemini/OpenAI) with keys in `SecureKeyStore`. No API keys in repo.

**Flow (v0.8.x):** track transition → `DjInterstitialGate` (1–2 songs) → optional transition ment → duck in/out → next scheduled track.

## Radio messenger UX harness (Phase F — v0.8.0)
...
| Local/JSON parser | `UserRequestParserTest`, `UserRequestParser.parseLocal` |
| Candidate pool | `CandidatePoolTest` — dedupe by search query |
| Play history | `PlayHistoryTest` — 20-track ring buffer |
| Scheduler | `RadioSchedulerTest` — pool pick, history skip, cushion |
| DJ interstitial gate | `DjInterstitialGateTest` — random 1–2 song cadence |
| Transition ment fallback | `DjAudioControlParserTest.fallbackForTransition` |
| DJ transition fixture | `harness/tests/mock_dj_transition.json` (also in `test_llm_response_schema.py`) |

**Flow:** ▶ Send → parse (no TTS) → candidate pool → auto schedule on idle/transition → optional DJ ment between tracks.
